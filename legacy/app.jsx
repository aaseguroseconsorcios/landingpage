...
function VantagensSection() {
  const items = [
  { num: '01', title: 'Sem juros', desc: 'Você paga só o crédito + taxa de administração.', feature: true,
    featureDesc: 'A diferença entre o consórcio e o financiamento pode chegar a 60% no valor final pago.' },
...
function App() {
  ...
  // Tweaks
  const defaults = window.TWEAK_DEFAULTS || { accent: 'red', heroVariant: 'default', showFab: true, showDots: true };
    ...
    <>
      ...
      <window.TweaksPanel title="Tweaks">
          ...
          <window.TweakSection title="UI">
            <window.TweakToggle label="Botão WhatsApp" value={tweaks.showFab} onChange={(v) => setTweak('showFab', v)} />
            <window.TweakToggle label="Indicador de seções" value={tweaks.showDots} onChange={(v) => setTweak('showDots', v)} />
          </window.TweakSection>
        </window.TweaksPanel>
      }
    </>);
